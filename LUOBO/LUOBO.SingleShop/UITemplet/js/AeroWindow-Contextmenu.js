...
(function (cm) {
    jQuery.fn.WinContextMenu = function (options) {
        ...
        cm(document).on('contextmenu', clickDOM.selector, function (e) {
            var target = e.currentTarget;
            var m = create(e).show();
            //var l = e.pageX + opt.offsetX,
            var l = e.clientX + opt.offsetX,
            //t = e.pageY + opt.offsetY,
            t = e.clientY + opt.offsetY,
			p = {
			    wh: cm(window).height(),
			    ww: cm(window).width(),
			    mh: m.height(),
            ...
            m.find('a').click(function (e) {//呼叫新从页面增加的菜单项
                var b = $(this).attr('ref');
                if (b != 'sitem') { this.className != 'cmDisable' ? opt.action(this, target) : null; }
                e.preventDefault();
            });
