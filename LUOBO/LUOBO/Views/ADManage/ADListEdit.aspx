                ...
                jQuery(obj[i]).click(function () {
                    ...
                    switch (o.target) {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            var _rel_c = jQuery(this).attr("class").substr(3);
...
<script type="text/javascript" language="javascript">
    ...
    <script type="text/javascript">
                ...
                success: function (obj) {
                    if (obj.ResultCode == "0") {
                        ...
                        for (var i = 0; i < obj.ResultOBJ.length; i++) {
                            ...
                            $("#templetfiles").append("<label class='btn btn-primary " + (i==0 ? " active":"") + "'><input type='radio' name='options' id='option1' onclick='javascript:getcontent(\"" + item.File_Name + "\");'>" + f + "</label>");
        ...
        function getcontent(page) {
            ...
            for (var i = 0; i < AD_TEMPLET.length; i++) {
                ...
                if (item.File_Name == page) {
                    ...
                    $("#PreviwPage").attr("src", item.File_Url+"?".concat(Math.random())  );
                    AD_TEMPPAGE = item;
                    $("#templetcontent").empty();
                    var templethtml = "";
                    var tmpvalue = "";
                    for (var j = 0; j < item.File_Templet.length; j++) {
                        if (item.File_Templet[j].Unit_Type == "txt") {
                            tmpvalue = "";
                            for (var i = 0; i < item.File_Content.length; i++) {
                                if (item.File_Content[i].TKey == ("Templet_" + j)) {
                                    tmpvalue = item.File_Content[i].TValue;
                                    break;
                                }
                            }
                        ...
                        if (item.File_Templet[j].Unit_Link == "true") {
                            tmpvalue = "";
                            for (var i = 0; i < item.File_Content.length; i++) {
                                if (item.File_Content[i].TKey == ("Templet_" + j + "_link")) {
                                    tmpvalue = item.File_Content[i].TValue;
                                    break;
                                }
                            }
                            templethtml += "<tr><td><div>连接地址：<input id='Templet_" + j + "_link' name='Templet_" + j + "_link' type='text' value='" + tmpvalue + "' /></div></td></tr>";
                        }
                    }
                ...
                case 2:
                    ...
                    if (tmpPubList != null) {
                        ...
                        for (var i = 0; i < tmpPubList.ACList.length; i++) {
                            ...
                            var checkstr = "√";
                            if (t) {
                                var checkstr = "<input type='checkbox' id='cb_pubids' name='cb_pubids' value='" + i + "'/>";
                            }
                            result = "<tr>";
                            result += "<td>" + checkstr + "</td>";
                    ...
                    if (pubList != null) {
                        ...
                        for (var i = 0; i < pubList.ACList.length; i++) {
                            ...
                            result = "<tr>";
                            result += "<td><input type='checkbox' id='cb_selectpubids' name='cb_selectpubids' value='" + i + "'/></td>";
                            result += "<td>" + item.CASE.AC_TITLE + "</td>";
                            result += "<td>" + item.SSID_Count + "</td>";
                            result += "<td>" + item.CASE.IS_COPYSSID.toString().replace("0", "否").replace("1", "是") + "</td>";
                            result += "<td>" + item.CASE.AD_TITLE + "</td>";
                            result += "</tr>";
                            $("#adpub_case_select").append(result);
                        }
        ...
        function subaudit() {
            ...
            alert(pubtype + "^^^^^^^^" + ids);
            postaudit($("#ad_id").val(), pubtype, ids, savecase);
        }
        function postaudit(adid, type, ids, savecase) {
            $.ajax({
                type: 'post',
                url: 'PostAudit',
                data: 'AD_ID=' + adid + "&AuditType=" + type + "&IDS=" + ids + "&ascase=" + savecase,
                dataType: 'json',
                success: function (obj) {
                    if (obj.ResultCode == "0") {
                        alert("ok");
                   ...
                   if (obj.ResultCode == "0") {
                      if (obj.ResultOBJ.APCTList.length > 0) {
                        var item;
                            ...
                            result = "<tr>";
                            result += "<td><a class='go-ad-view' onclick='javascript:getmain(" + item.AD_ID + ")'><i class='fa fa-fw fa-rss'></i>" + item.AD_SSID + "</a></td>";
                            result += "<td>" + getStaStr(item.AD_Stat) + "</td>";
                            result += "<td class='hidden-xs actions'><div class='action-buttons'><a href='javascript:getmain(" + item.AD_ID + ")'><i class='fa fa-fw fa-pencil'></i></a><a href='ADPubHistory?ADID=" + item.AD_ID + "'><i class='fa fa-fw fa-code-fork'></i></a><a href='javascript:DelAD(" + item.AD_ID + ")'><i class='fa fa-fw fa-trash-o'></i></a></div></td>";
                            result += "</tr>";
      ...
      <div  class="row-fluid">
        <div class="div-body col-md-4" id="ad-list" name="ad-list">
          <div class="widget-container  clearfix">
            ...
            <select id="search_type" name="search_type" onchange="changetype()";><option value="-1">全部</option><option value="0">编辑</option><option value="1">审核中</option><option value="2">已审核</option></select>
