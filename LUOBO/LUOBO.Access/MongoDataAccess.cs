...
{
    public class MongoDataAccess : IDisposable
    {
        #region 数据库参数
        ...
        {
            _connectionstr = ConfigurationSettings.AppSettings["MongoConn"];
            _dbName = ConfigurationSettings.AppSettings["MongoDB"];
            _server = MongoDB.Driver.MongoServer.Create(_connectionstr);
        ...
        /// <param name="list">需要插入数据的列表</param>
        /// <param name="collectionName">指定要插入的集合</param>
        public void Insert(List<string> list, string collectionName)
        {
            MongoCollection<BsonDocument> mc = this._db.GetCollection<BsonDocument>(collectionName);
            List<BsonDocument> bsonList = new List<BsonDocument>();
            list.ForEach(t => bsonList.Add(BsonDocument.Parse(t)));
            mc.InsertBatch(bsonList);
        }
        #endregion
